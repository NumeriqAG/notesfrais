const CACHE_NAME = 'notesfrais-mike-shell-v64';
...
self.addEventListener('activate', event => {
  event.waitUntil(
    caches.keys()
      .then(keys => Promise.all(keys.filter(key => key.startsWith('notesfrais-mike-') && key !== CACHE_NAME).map(key => caches.delete(key))))
