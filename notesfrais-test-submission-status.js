(function(){
  ...
  window.patchNotesFrais = function(html){
    ...
    if(html.includes('NOTESFRAIS_SUBMISSION_STATUS_TEST_V1')) return html;

    const helpers = String.raw`
const NOTESFRAIS_SUBMISSION_STATUS_TEST_V1=true;
...
`;
  ...
  const submitMonth=useCallback(async()=>{
    ...
    html = html.replace(
      ...
      "<div style={{fontSize:13,color:'var(--t3)'}}>Tableau de bord des frais professionnels</div><div style={{display:'inline-flex',alignItems:'center',gap:7,marginTop:10,background:monthSubmissionMeta.bg,color:monthSubmissionMeta.color,borderRadius:999,padding:'7px 11px',fontSize:12,fontWeight:800}}><span>{monthSubmissionMeta.icon}</span><span>{monthSubmissionMeta.label}</span><span style={{fontWeight:500,opacity:0.78}}>· {monthSubmissionMeta.hint}</span></div>"
    );
